  ...
  const faqItems: FAQItem[] = [
    {
      ...
      answer:
        "API tokens are managed by the project maintainers. Please contact one of the maintainers listed below via GitHub or email. Include your GitHub username and a brief description of what you'll be using the token for (e.g., running benchmarks on a specific fork, CI integration, etc.).",
      icon: Key,
    },
    {
      question: 'How do I create a new environment?',
      answer:
        'Environments represent different hardware/OS combinations where benchmarks run. To add a new environment, contact the maintainers with details about your system (OS, CPU architecture, memory, etc.). They will create the environment configuration for you.',
      icon: Settings,
    },
    {
      question: 'How do I add a new binary configuration?',
      answer:
        'Binary configurations represent different CPython build flags (e.g., --enable-optimizations, --with-lto). To add a new configuration, open a GitHub issue or contact the maintainers with the specific build flags you want to test.',
      icon: Package,
    },
    {
      question: 'Why is my benchmark data not showing up?',
      answer:
        'There could be several reasons: 1) The benchmark worker might still be processing your commits, 2) There might be an issue with your API token permissions, 3) The specified binary/environment combination might not exist. Check the worker logs or contact the maintainers for help.',
      icon: HelpCircle,
    },
    {
      question: 'How often are benchmarks run?',
      answer:
        'Benchmarks are automatically run for each commit to the main CPython repository by community members running worker processes. Anyone with an API token can run additional benchmarks on feature branches or custom commits using the worker tool.',
      icon: BarChart3,
    },
    {
      question: 'Can I run benchmarks on my own fork?',
      answer:
        'Yes! Anyone can run the memory-tracker worker tool with an API token to benchmark any CPython fork or branch. The worker tool is open source and designed to be run by community members. See the worker documentation for setup instructions.',
      icon: GitBranch,
    },
  ];
  return (
    <div className="container py-12 max-w-6xl">
      ...
      <div className="mb-16">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-12">
          ...
          <Card className="group hover:shadow-lg transition-all duration-300 hover:-translate-y-1">
            ...
            <CardContent>
              <p className="text-muted-foreground">
                Community members running worker processes monitor the CPython
                repository for new commits. When a commit is pushed, workers can
                queue it for memory profiling across multiple configurations and
                environments.
              </p>
            </CardContent>
          </Card>
          ...
          <Card className="group hover:shadow-lg transition-all duration-300 hover:-translate-y-1">
            ...
            <CardContent>
              <p className="text-muted-foreground">
                Worker processes (which anyone can run with an API token)
                compile CPython with various optimization flags, then run a
                comprehensive suite of memory benchmarks using Memray. Each
                benchmark captures detailed allocation data, call stacks, and
          ...
          <Card className="group hover:shadow-lg transition-all duration-300 hover:-translate-y-1">
            ...
            <CardContent>
              <p className="text-muted-foreground">
                Results are processed and stored with rich metadata. The web
                interface provides interactive visualizations, trend analysis,
                and detailed flamegraphs to help developers understand memory
                behavior and identify regressions.
              </p>
            </CardContent>
          </Card>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <Card className="group hover:shadow-xl transition-all duration-500 hover:-translate-y-2 hover:rotate-1 border-l-4 border-l-primary/30 hover:border-l-primary">
            ...
            <CardContent>
              <p className="text-muted-foreground">
                CPython commits are profiled by community workers across
                multiple build configurations. Track memory usage trends over
                time, identify sudden regressions, and validate that memory
          ...
          <Card className="group hover:shadow-xl transition-all duration-500 hover:-translate-y-2 hover:-rotate-1 border-l-4 border-l-primary/30 hover:border-l-primary">
            ...
            <CardContent>
              <p className="text-muted-foreground">
                Compare memory usage across different build flags
                (--enable-optimizations, --with-lto, debug builds), Python
                versions, and hardware environments. Understand how different
      ...
      <Card className="mb-12 border-primary/20 bg-primary/5 hover:shadow-2xl transition-all duration-700 relative overflow-hidden group">
        ...
        <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-700">
          <div
            className="absolute top-4 left-4 w-16 h-16 border-2 border-pink-400/30 rounded-full animate-spin"
            style={{ animationDuration: '4s' }}
          ></div>
          <div
            className="absolute top-8 right-8 w-12 h-12 border-2 border-cyan-400/30 rounded-full animate-spin"
            style={{ animationDuration: '3s', animationDirection: 'reverse' }}
          ></div>
          <div
            className="absolute bottom-6 left-12 w-20 h-20 border-2 border-purple-400/30 rounded-full animate-spin"
            style={{ animationDuration: '5s' }}
          ></div>
          <div
            className="absolute bottom-12 right-6 w-8 h-8 border-2 border-yellow-400/30 rounded-full animate-spin"
            ...
            >
              <CollapsibleTrigger className="flex items-center justify-between w-full p-4 rounded-lg hover:bg-muted hover:shadow-md transition-all duration-300 group hover:-translate-y-0.5">
                <div className="flex items-center gap-3 text-left">
                  {item.icon && (
                    <item.icon className="h-5 w-5 text-primary flex-shrink-0 group-hover:scale-110 transition-transform" />
                  )}
              ...
              <CollapsibleContent className="px-4 pb-4">
                <div className="pl-8 text-muted-foreground">{item.answer}</div>
              </CollapsibleContent>
            </Collapsible>
          ))}
        </CardContent>
