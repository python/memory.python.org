...
async def init_admin_data():
    ...
    try:
        async for db in get_database():
            ...
            existing_admin = await crud.get_admin_user_by_username(
                db, settings.admin_initial_username
            )
            if existing_admin:
                print(
                    f"✅ Admin user '{settings.admin_initial_username}' already exists"
                )
                return True
...
async def init_database():
    ...
    try:
        ...
        print(
            f"Connected to: {settings.database_url.split('@')[-1] if '@' in settings.database_url else settings.database_url}"
        )
...
if __name__ == "__main__":
    ...
    if args.database_url:
        ...
        settings = get_settings()
        print(
            f"Using database URL: {args.database_url.split('@')[-1] if '@' in args.database_url else args.database_url}"
        )
        print(
            f"Settings database URL: {settings.database_url.split('@')[-1] if '@' in settings.database_url else settings.database_url}"
        )
